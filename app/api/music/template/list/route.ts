...
export async function GET(request: Request) {
  try {
    console.log("=== Music Template List API Called ===");
    ...
    const url = new URL(API_ENDPOINTS.MUSIC_TEMPLATE_LIST);
    if (monthNumber) {
    ...
    if (data.code === 0) {
      ...
      return NextResponse.json(
        {
          success: true,
          message: "Music template list fetched successfully",
    ...
    console.error("Music template list fetch error:", error);
    return NextResponse.json(
      { error: "Failed to fetch music template list" },
      { status: 500 },
    );
  }
}
